...
func (d *Database) GetTaskList(id int64) (*TaskList, error) {
	row := d.db.QueryRow(`SELECT id, name, group_jid, admin_jid, status, created_at FROM task_lists WHERE id = ?`, id)
	tl := &TaskList{}
	err := row.Scan(&tl.ID, &tl.Name, &tl.GroupJID, &tl.AdminJID, &tl.Status, &tl.CreatedAt)
	if err != nil {
		return nil, err
	}
	return tl, nil
...
func (d *Database) GetTaskListByNameAndGroup(name, groupJID string) (*TaskList, error) {
	row := d.db.QueryRow(`SELECT id, name, group_jid, admin_jid, status, created_at FROM task_lists WHERE name = ? AND group_jid = ?`, name, groupJID)
	tl := &TaskList{}
	err := row.Scan(&tl.ID, &tl.Name, &tl.GroupJID, &tl.AdminJID, &tl.Status, &tl.CreatedAt)
	if err != nil {
		return nil, err
...
func (d *Database) GetListsByGroupAndAdmin(groupJID, adminJID string) ([]TaskList, error) {
	rows, err := d.db.Query(
		`SELECT id, name, group_jid, admin_jid, status, created_at FROM task_lists WHERE group_jid = ? AND admin_jid = ?`,
		groupJID, adminJID,
	)
	if err != nil {
		return nil, err
	...
	for rows.Next() {
		var tl TaskList
		rows.Scan(&tl.ID, &tl.Name, &tl.GroupJID, &tl.AdminJID, &tl.Status, &tl.CreatedAt)
...
func (d *Database) GetActiveListsByGroup(groupJID string) ([]TaskList, error) {
	rows, err := d.db.Query(
		`SELECT id, name, group_jid, admin_jid, status, created_at FROM task_lists WHERE group_jid = ? AND status IN ('unpublished', 'active')`,
		groupJID,
	)
	if err != nil {
		return nil, err
...
func (d *Database) GetActiveListsByGroupAndAdmin(groupJID, adminJID string) ([]TaskList, error) {
	rows, err := d.db.Query(
		`SELECT id, name, group_jid, admin_jid, status, created_at FROM task_lists WHERE group_jid = ? AND admin_jid = ? AND status IN ('unpublished', 'active')`,
		groupJID, adminJID,
	)
	if err != nil {
		return nil, err
...
func (d *Database) GetAllActiveLists() ([]TaskList, error) {
	rows, err := d.db.Query(
		`SELECT id, name, group_jid, admin_jid, status, created_at FROM task_lists WHERE status = 'active'`,
	)
	if err != nil {
		return nil, err
	}
